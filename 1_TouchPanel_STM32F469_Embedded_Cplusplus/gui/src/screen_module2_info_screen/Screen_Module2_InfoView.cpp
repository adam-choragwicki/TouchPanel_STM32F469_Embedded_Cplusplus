...
Screen_Module2_InfoView::Screen_Module2_InfoView()
{
#ifndef SIMULATOR    
  ...
  for(int i = 0; i < INIT_PARAMETER_NAME_COUNT; i++)
  {
    ...
    {
      initParameterNameStringDisplay[i][character] = Model::m_InitParametersModule2[i + INIT_PARAMETER_NAME_OFFSET][character];
    }
  }
  ...
  for(int i = 0; i < INIT_PARAMETER_VALUE_COUNT; i++)
  {
    for (int character = 0; character < PAYLOAD_SIZE; character++)
    {
      initParameterValueStringDisplay[i][character] = Model::m_InitParametersModule2[i + INIT_PARAMETER_VALUE_OFFSET][character];
    }
  }
...
void Screen_Module2_InfoView::UpdateCpuUsage(uint8_t value)
{  
