...
Screen_Module3_InfoView::Screen_Module3_InfoView()
{
#ifndef SIMULATOR    
  ...
  for(int i = 0; i < INIT_PARAMETER_NAME_COUNT; i++)
  {
    ...
    {
      initParameterNameStringDisplay[i][character] = Model::m_InitParametersModule3[i + INIT_PARAMETER_NAME_OFFSET][character];
    }
  }
  ...
  for(int i = 0; i < INIT_PARAMETER_VALUE_COUNT; i++)
  {
    for (int character = 0; character < PAYLOAD_SIZE; character++)
    {
      initParameterValueStringDisplay[i][character] = Model::m_InitParametersModule3[i + INIT_PARAMETER_VALUE_OFFSET][character];
    }
  }
